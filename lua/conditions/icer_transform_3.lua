local function data(ch)
    return { bonus = 45000000, mult = 40000, name = "@YTransform @WThird@n" }
end
...
local function modifiers(ch)
    ...
    return {
        ...
        { target = { "derived", "height" }, kind = "multiplier", value = 15000, label = form.name },
        { target = { "derived", "weight" }, kind = "multiplier", value = 20000, label = form.name },
    }
end
