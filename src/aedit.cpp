...
ACMD(do_oasis_aedit) {
  ...
  if (!strcasecmp("save", arg)) {
    mudlog(CMP, MAX(ADMLVL_BUILDER, GET_INVIS_LEV(ch)), TRUE,
           "OLC: %s saves socials.", GET_NAME(ch));
  ...
  if (OLC_ZNUM(d) > top_of_socialt) {
    ...
    send_to_char(ch, "Do you wish to edit the '%s' action? ",
  ...
  mudlog(CMP, ADMLVL_IMMORT, TRUE, "OLC: %s starts editing actions.",
         GET_NAME(ch));
}
...
void aedit_setup_existing(struct descriptor_data *d, int real_num) {
  ...
  if (soc_mess_list[real_num].others_no_arg)
    OLC_ACTION(d)->others_no_arg =
        strdup(soc_mess_list[real_num].others_no_arg);
...
void aedit_save_to_disk(struct descriptor_data *d) {
  ...
  if (!(fp = fopen(SOCMESS_FILE_NEW, "w+"))) {
    ...
    snprintf(error, sizeof(error), "Can't open socials file '%s'",
             SOCMESS_FILE);
    perror(error);
    exit(1);
  }
  ...
  for (i = 0; i <= top_of_socialt; i++) {
    fprintf(fp, "~%s %s %d %d %d %d\n", soc_mess_list[i].command,
            soc_mess_list[i].sort_as, soc_mess_list[i].hide,
    ...
    fprintf(
        ...
        ((soc_mess_list[i].char_body_found) ? soc_mess_list[i].char_body_found
                                            : "#"),
        ((soc_mess_list[i].others_body_found)
             ? soc_mess_list[i].others_body_found
             : "#"),
    ...
    fprintf(fp, "%s\n%s\n\n",
            ((soc_mess_list[i].char_obj_found) ? soc_mess_list[i].char_obj_found
                                               : "#"),
            ((soc_mess_list[i].others_obj_found)
                 ? soc_mess_list[i].others_obj_found
                 : "#"));
  }
...
void aedit_disp_menu(struct descriptor_data *d) {
  ...
  write_to_output(
      d,
      "@n-- Action editor\r\n"
      "@gn@n) Command         : @y%-15.15s@n @g1@n) Sort as Command  : "
      "@y%-15.15s@n\r\n"
      "@g2@n) Min Position[CH]: @c%-8.8s        @g3@n) Min Position [VT]: "
      "@c%-8.8s\r\n"
      "@g4@n) Min Level   [CH]: @c%-3d             @g5@n) Show if Invisible: "
      "@c%s\r\n"
      ...
      position_types[action->min_victim_position], action->min_level_char,
...
void aedit_parse(struct descriptor_data *d, char *arg) {
  ...
  case AEDIT_MAIN_MENU:
    ...
    case '1':
      write_to_output(
          d, "Enter sort info for this action (for the command listing): ");
      OLC_MODE(d) = AEDIT_SORT_AS;
      return;
    case '2':
      write_to_output(d, "Enter the minimum position the Character has to be "
                         "in to activate social:\r\n");
      {
      ...
      return;
    case '3':
      write_to_output(d, "Enter the minimum position the Victim has to be in "
                         "to activate social:\r\n");
      {
        for (i = POS_DEAD; i <= POS_STANDING; i++)
          write_to_output(d, "   %d) %s\r\n", i, position_types[i]);
    ...
    case 'A':
      write_to_output(
          d,
          "Enter social shown to the Character when there is no argument "
          "supplied.\r\n"
          "[OLD]: %s\r\n"
          "[NEW]: ",
    ...
    case 'B':
      write_to_output(
          ...
          "[OLD]: %s\r\n"
          "[NEW]: ",
          ((OLC_ACTION(d)->others_no_arg) ? OLC_ACTION(d)->others_no_arg
                                          : "NULL"));
    ...
    case 'F':
      write_to_output(
          d,
          "Enter normal social shown to the Character when the victim is "
          "found.\r\n"
          "[OLD]: %s\r\n"
          "[NEW]: ",
    ...
    case 'G':
      write_to_output(
          ...
          "[OLD]: %s\r\n"
          "[NEW]: ",
          ((OLC_ACTION(d)->others_found) ? OLC_ACTION(d)->others_found
                                         : "NULL"));
    ...
    case 'H':
      write_to_output(
          d,
          "Enter normal social shown to the Victim when the victim is "
          "found.\r\n"
          "[OLD]: %s\r\n"
          "[NEW]: ",
    ...
    case 'I':
      write_to_output(d,
                      "Enter 'body part' social shown to the Character when "
                      "the victim is found.\r\n"
                      "[OLD]: %s\r\n"
                      "[NEW]: ",
                      ((OLC_ACTION(d)->char_body_found)
    ...
    case 'J':
      write_to_output(d,
                      "Enter 'body part' social shown to Others when the "
                      "victim is found.\r\n"
                      "[OLD]: %s\r\n"
                      "[NEW]: ",
                      ((OLC_ACTION(d)->others_body_found)
    ...
    case 'K':
      write_to_output(d,
                      "Enter 'body part' social shown to the Victim when the "
                      "victim is found.\r\n"
                      "[OLD]: %s\r\n"
                      "[NEW]: ",
                      ((OLC_ACTION(d)->vict_body_found)
    ...
    case 'L':
      write_to_output(d,
                      "Enter 'object' social shown to the Character when the "
                      "object is found.\r\n"
                      "[OLD]: %s\r\n"
                      "[NEW]: ",
                      ((OLC_ACTION(d)->char_obj_found)
    ...
    case 'M':
      write_to_output(d,
                      "Enter 'object' social shown to the Room when the object "
                      "is found.\r\n"
                      "[OLD]: %s\r\n"
                      "[NEW]: ",
                      ((OLC_ACTION(d)->others_obj_found)
...
ACMD(do_astat) {
  ...
  send_to_char(
      ...
      "2) Min Position[CH]: @c%-8.8s@n        3) Min Position[VT]: "
      "@c%-8.8s@n\r\n"
      "4) Min Level   [CH]: @c%-3d@n             5) Show if Invis   : "
      "@c%s@n\r\n"
      "a) Char    [NO ARG]: @c%s@n\r\n"
