...
int load_char(const char *name, struct char_data *ch) {
  ...
  if (!ch->time.birth) {
    log("No birthday for user %s, using standard starting age determination",
        GET_NAME(ch));
    ch->time.birth = time(0) - birth_age(ch);
  }

  if (!ch->time.maxage) {
    log("No max age for user %s, using standard max age determination",
        GET_NAME(ch));
...
void remove_player(int pfilepos) {
  ...
  log("PCLEAN: %s Lev: %d Last: %s", player_table[pfilepos].name,
