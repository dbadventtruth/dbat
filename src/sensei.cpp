...
Sensei::Sensei(sensei_id sid, const std::string &name, std::string abbr,
               std::string style) {
  this->s_id = sid;
...
sensei_id Sensei::getID() const { return s_id; }

const std::string &Sensei::getAbbr() const { return abbr; }

const std::string &Sensei::getName() const { return name; }

const std::string &Sensei::getNameLower() const { return lower_name; }

const std::string &Sensei::getStyleName() const { return style; }
...
void load_sensei() {
  ...
  sensei_map[kurzak] =
      new Sensei(kurzak, "Kurzak", "Kurzak", "Adaptive Taijutsu");
...
SenseiMap valid_for_race_pc(char_data *ch) {
  ...
  for (const auto &s : sensei_map) {
    if (s.second->senseiIsPcOk() &&
        s.second->senseiAvailableForRace(get_race(ch->race)->getID())) {
