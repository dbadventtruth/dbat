...
void gedit_save_to_disk(struct zone_data *zone) { save_guilds(zone); }
...
void gedit_setup_new(struct descriptor_data *d) {
  ...
  G_NO_GOLD(guilddata) =
      strdup("%s Sorry, but I'm gonna need more zenni first.");
  ...
  case GEDIT_MAIN_MENU:
    ...
    case 'Q':
      if (OLC_VAL(d)) { /*. Anything been changed? . */
        write_to_output(
            d, "Do you wish to save the changes to the Guild? (y/n) : ");
