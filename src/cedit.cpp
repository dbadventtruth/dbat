...
int save_config(IDXTYPE nowhere) {
  ...
  fprintf(
      ...
      "*\n"
      "* - If variable is a yes/no or true/false based variable, use 1's and "
      "0's\n"
  ...
  fprintf(
      fl,
      "* Admin level and above of players whom are immune to idle penalties.\n"
      "idle_max_level = %d\n\n",
  ...
  fprintf(fl,
          "* When an immortal loads an object, should it load into their "
          "inventory?\n"
  ...
  fprintf(fl,
          "* What is the level that cannot be attained?\n"
          "level_cap = %d\n\n",
          CONFIG_LEVEL_CAP);
  fprintf(fl,
  ...
  fprintf(fl,
          ...
          CONFIG_STACK_OBJS);
  fprintf(fl,
          "* Allow aggressive mobs to attack other mobs?\n"
          "mob_fighting = %d\n\n",
          CONFIG_MOB_FIGHTING);
  fprintf(fl,
  ...
  fprintf(fl,
          "* How many points in players initial points pool if using point "
          "pool creation?\n"
          "initial_points = %d\n\n",
  ...
  fprintf(fl,
          "* Should the MUD allow you to 'rent' for free?  (i.e. if you just "
          "quit,\n"
  ...
  fprintf(fl,
          "* If auto_save = 1, how often (in minutes) should the game save "
          "people's objects?\n"
          "autosave_time = %d\n\n",
  ...
  fprintf(
      ...
      "* sequentially (1 & 2 before 3). If you don't, you might not be able "
      "to\n"
  ...
  fprintf(fl,
          "* Maximum number of players allowed before game starts to turn "
          "people away.\n"
          "max_playing = %d\n\n",
          CONFIG_MAX_PLAYING);

  fprintf(fl,
          "* Maximum size of bug, typo, and idea files in bytes (to prevent "
          "bombing).\n"
          "max_filesize = %d\n\n",
          CONFIG_MAX_FILESIZE);
  ...
  fprintf(
      fl,
      "* Should the game automatically create a new wizlist/immlist every "
      "time\n"
  ...
  fprintf(fl,
          "* Should characters be allowed to advance in multiple classes?\n"
          "allow_multiclass = %d\n\n",
          CONFIG_ALLOW_MULTICLASS);

  fprintf(fl,
          "* Should characters be allowed to advance in prestige classes?\n"
          "allow_prestige = %d\n\n",
  ...
  fprintf(fl,
          "*Speed of Violence system. 0 - 10 (10) being the slowest.\n"
          "pulse_viol = %d\n\n",
          CONFIG_PULSE_VIOLENCE);

  fprintf(fl,
          "*Speed of Mobile actions. 0 - 20 (20) being the slowest.\n"
          "pulse_mobile = %d\n\n",
          CONFIG_PULSE_MOBILE);

  fprintf(fl,
          "*Zone updates. 0 - 20 (20) being the slowest.\n"
          "pulse_zone = %d\n\n",
          CONFIG_PULSE_ZONE);

  fprintf(fl,
          "*Autosave. 0 - 100 (100) being the slowest.\n"
          "pulse_autosave = %d\n\n",
          CONFIG_PULSE_AUTOSAVE);

  fprintf(fl,
          "*Idling passwords. 0 - 30 (30) being the slowest.\n"
          "pulse_idlepwd = %d\n\n",
          CONFIG_PULSE_IDLEPWD);

  fprintf(fl,
          "*Sanity check time.. 0 - 50 (50) being the slowest.\n"
          "pulse_sanity = %d\n\n",
          CONFIG_PULSE_SANITY);

  fprintf(fl,
          "*Usage time.  0 - 10 (10) being the slowest in minutes.\n"
          "pulse_usage = %d\n\n",
          CONFIG_PULSE_USAGE);

  fprintf(fl,
          "*Timesaving. 0 - 50 (50) being the slowest in minutes.\n"
          "pulse_timesave = %d\n\n",
          CONFIG_PULSE_TIMESAVE);

  fprintf(fl,
          "*Current update time. 0 - 30 (30) being the slowest.\n"
          "pulse_current = %d\n\n",
...
void cedit_disp_menu(struct descriptor_data *d) {
   ...
   * Menu header
   */
  write_to_output(d, "OasisOLC MUD Configuration Editor\r\n"
...
void cedit_disp_game_play_options(struct descriptor_data *d) {
  ...
  write_to_output(
      ...
      OLC_CONFIG(d)->play.level_can_shout, OLC_CONFIG(d)->play.holler_move_cost,
      OLC_CONFIG(d)->play.tunnel_size, OLC_CONFIG(d)->play.max_exp_gain,
      OLC_CONFIG(d)->play.max_exp_loss, OLC_CONFIG(d)->play.max_npc_corpse_time,
      OLC_CONFIG(d)->play.max_pc_corpse_time,

      OLC_CONFIG(d)->play.idle_void, OLC_CONFIG(d)->play.idle_rent_time,
      ...
      CHECK_VAR(OLC_CONFIG(d)->play.mob_fighting), OLC_CONFIG(d)->play.OK,
      OLC_CONFIG(d)->play.NOPERSON, OLC_CONFIG(d)->play.NOEFFECT,
...
void cedit_disp_crash_save_options(struct descriptor_data *d) {
  ...
  write_to_output(
      ...
      "@WQ@B) @CExit To The Main Menu\r\n"
      "@WEnter your choice : @n",
      CHECK_VAR(OLC_CONFIG(d)->csd.free_rent), OLC_CONFIG(d)->csd.max_obj_save,
      OLC_CONFIG(d)->csd.min_rent_cost, CHECK_VAR(OLC_CONFIG(d)->csd.auto_save),
      OLC_CONFIG(d)->csd.autosave_time, OLC_CONFIG(d)->csd.crash_file_timeout,
...
void cedit_disp_operation_options(struct descriptor_data *d) {
  ...
  write_to_output(
      ...
      OLC_CONFIG(d)->operation.DFLT_IP ? OLC_CONFIG(d)->operation.DFLT_IP
                                       : "<None>",
      OLC_CONFIG(d)->operation.DFLT_DIR ? OLC_CONFIG(d)->operation.DFLT_DIR
                                        : "<None>",
      OLC_CONFIG(d)->operation.LOGNAME ? OLC_CONFIG(d)->operation.LOGNAME
                                       : "<None>",
      ...
      OLC_CONFIG(d)->operation.WELC_MESSG ? OLC_CONFIG(d)->operation.WELC_MESSG
                                          : "<None>",
...
void cedit_disp_ticks_menu(struct descriptor_data *d) {
  ...
  write_to_output(
      ...
      "@WQ@B) @CExit To The Main Menu\r\n"
      "@WEnter your choice : @n",
      OLC_CONFIG(d)->ticks.pulse_violence, OLC_CONFIG(d)->ticks.pulse_mobile,
      OLC_CONFIG(d)->ticks.pulse_zone, OLC_CONFIG(d)->ticks.pulse_autosave,
      OLC_CONFIG(d)->ticks.pulse_idlepwd, OLC_CONFIG(d)->ticks.pulse_sanity,
      OLC_CONFIG(d)->ticks.pulse_usage, OLC_CONFIG(d)->ticks.pulse_timesave,
...
void cedit_disp_creation_options(struct descriptor_data *d) {
  ...
  write_to_output(d,
                  ...
                  "@WC@B) @CNumber of points in initial points pool for pool "
                  "generation methods : @c%d \r\n"
                  "@WQ@B) @CExit To The Main Menu\r\n"
                  "@WEnter your choice : @n",
...
void cedit_parse(struct descriptor_data *d, char *arg) {
  ...
  case CEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      mudlog(CMP, MAX(ADMLVL_BUILDER, GET_INVIS_LEV(d->character)), TRUE,
             "OLC: %s modifies the game configuration.",
  ...
  case CEDIT_GAME_OPTIONS_MENU:
    ...
    case 'D':
      write_to_output(
          d, "Enter the amount it costs (in move points) to holler : ");
    ...
    case 'E':
      write_to_output(
          d, "Enter the maximum number of people allowed in a tunnel : ");
    ...
    case 'F':
      write_to_output(
          d, "Enter the maximum gain of experience per kill for players : ");
    ...
    case 'G':
      write_to_output(
          d, "Enter the maximum loss of experience per death for players : ");
    ...
    case 'H':
      write_to_output(
          d, "Enter the number of tics before NPC corpses decompose : ");
    ...
    case 'I':
      write_to_output(
          d, "Enter the number of tics before PC corpses decompose : ");
      OLC_MODE(d) = CEDIT_MAX_PC_CORPSE_TIME;
      return;

    case 'j':
    case 'J':
      write_to_output(d, "Enter the number of tics before PC's are sent to the "
                         "void (idle) : ");
    ...
    case 'k':
    case 'K':
      write_to_output(d, "Enter the number of tics before PC's are "
                         "automatically rented and forced to quit : ");
    ...
    case 'L':
      write_to_output(
          d, "Enter the level a player must be to become immune to IDLE : ");
    ...
    case 'T':
      write_to_output(
          d,
          "Enter the multiplier a player will recieve to experience gained : ");
  ...
  case CEDIT_CRASHSAVE_OPTIONS_MENU:
    ...
    case 'B':
      write_to_output(d,
                      "Enter the maximum number of items players can rent : ");
    ...
    case 'E':
      write_to_output(
          d, "Enter how often (in minutes) should the MUD save players : ");
    ...
    case 'F':
      write_to_output(
          d, "Enter the lifetime of crash and idlesave files (days) : ");
  ...
  case CEDIT_ROOM_NUMBERS_MENU:
    ...
    case 'A':
      write_to_output(
          d, "Enter the room's vnum where mortals should load into : ");
    ...
    case 'B':
      write_to_output(
          d, "Enter the room's vnum where immortals should load into : ");
    ...
    case 'C':
      write_to_output(
          d, "Enter the room's vnum where frozen people should load into : ");
  ...
  case CEDIT_OPERATION_OPTIONS_MENU:
    ...
    case 'J':
      ...
      send_to_char(d->character, "Please note that using the stock social file "
                                 "will disable AEDIT.\r\n");
      break;
    ...
    case 'L':
      ...
      string_write(d, &OLC_CONFIG(d)->operation.MENU, MAX_INPUT_LENGTH, 0,
                   oldtext);
      return;
    ...
    case 'M':
      ...
      string_write(d, &OLC_CONFIG(d)->operation.WELC_MESSG, MAX_INPUT_LENGTH, 0,
                   oldtext);
      return;
    ...
    case 'N':
      ...
      string_write(d, &OLC_CONFIG(d)->operation.START_MESSG, MAX_INPUT_LENGTH,
                   0, oldtext);
      return;
    case 'o':
    case 'O':
  ...
  case CEDIT_AUTOWIZ_OPTIONS_MENU:
    ...
    case 'B':
      write_to_output(
          d, "Enter the minimum level for players to appear on the wizlist : ");
  ...
  case CEDIT_TICKS_OPTIONS_MENU:
    ...
    case 'A':
      write_to_output(
          d, "Enter the Speed of the violence system (1 - 10) 10 = Slowest : ");
    ...
    case 'B':
      write_to_output(
          d, "Enter the Speed of the Mobile Actions (1 - 20) 20 = Slowest : ");
    ...
    case 'E':
      write_to_output(
          d, "Enter the time to kill connection waiting for password : ");
  ...
  case CEDIT_IDLE_VOID:
    if (!*arg) {
      write_to_output(d, "That is an invalid choice!\r\n"
                         "Enter the number of tics before PC's are sent to the "
                         "void (idle) : ");
    } else {
  ...
  case CEDIT_IDLE_RENT_TIME:
    if (!*arg) {
      write_to_output(d, "That is an invalid choice!\r\n"
                         "Enter the number of tics before PC's are "
                         "automatically rented and forced to quit : ");
    } else {
  ...
  case CEDIT_CREATION_MENU:
    if (!isdigit(*arg) || atoi(arg) < 0 ||
        atoi(arg) > NUM_CREATION_METHODS - 1) {
