    ...
    if (strncasecmp("new", buf, 3) == 0) {
      if (!isdigit(*buf2)) {
        ...
        send_to_char(d->character,
                     "You need to create the assembly object before you can "
...
void assedit_setup(struct descriptor_data *d, int number) {
   ...
   * of all the data contained in the original assembly structure.
   */

  if ((lRnum = real_object(OLC_ASSEDIT(d)->lVnum)) < 0) {
    send_to_char(d->character, "Assembled item may not exist, check the vnum "
                               "and assembles (show assemblies). \r\n");
...
void assedit_disp_menu(struct descriptor_data *d) {
  ...
  if (OLC_ASSEDIT(d)->lNumComponents <= 0)
    ...
    for (i = 0; i < OLC_ASSEDIT(d)->lNumComponents; i++) {
      if (auto proto = obj_proto_by_id(OLC_ASSEDIT(d)->pComponents[i].lVnum);
          !proto) {
        send_to_char(d->character,
                     "@g%2d@n) @y ERROR --- Contact an Implementor @n\r\n ",
                     i + 1);
      } else {
        send_to_char(d->character,
                     "@g%2d@n) [@c%5ld@n] %-20.20s  In room: @c%-3.3s@n    "
                     "Extract: @y%-3.3s@n\r\n",
  ...
  send_to_char(d->character, "@gA@n) Add a new component.\r\n"
...
void assedit_parse(struct descriptor_data *d, char *arg) {
  ...
  case ASSEDIT_ADD_COMPONENT: /* add a new component */
    if (isdigit(*arg)) {
      ...
      OLC_ASSEDIT(d)->pComponents[OLC_ASSEDIT(d)->lNumComponents].bExtract =
          YES;
      ...
      send_to_char(d->character,
                   "That object does not exist. Please try again\r\n");
      assedit_disp_menu(d);
    }
    break;
  ...
  case ASSEDIT_EDIT_EXTRACT:
    ...
    default:
      send_to_char(
          d->character,
          "Is the item to be extracted when the assembly is created? (Y/N)");
      break;
    }
    break;
  case ASSEDIT_EDIT_INROOM:
    ...
    default:
      send_to_char(
          d->character,
          "Object in the room when assembly is created? (n =  in inventory):");
      break;
    }
    break;
  ...
  default: /* default for whole assedit parse function */
    ...
    mudlog(BRF, ADMLVL_GOD, TRUE,
           "SYSERR: OLC assedit_parse(): Reached default case!");
...
void assedit_delete(struct descriptor_data *d) {
  send_to_char(d->character,
               "Which item number do you wish to delete from this assembly?");
  OLC_MODE(d) = ASSEDIT_DELETE_COMPONENT;
  return;
}
...
void assedit_edit_inroom(struct descriptor_data *d) {
  send_to_char(d->character, "Should the object be in the room when assembly "
                             "is created (n = in inventory)?");
