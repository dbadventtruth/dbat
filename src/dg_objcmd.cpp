...
void obj_log(obj_data *obj, const char *format, ...) {
  ...
  snprintf(output, sizeof(output), "Obj (%s, VNum %d):: %s",
           obj->short_description, GET_OBJ_VNUM(obj), format);
...
OCMD(do_dupe) { SET_BIT_AR(GET_OBJ_EXTRA(obj), ITEM_DUPLICATE); }
...
OCMD(do_osetval) {
  ...
  two_arguments(argument, arg1, arg2);
  if (arg1 == NULL || !*arg1 || arg2 == NULL || !*arg2 || !is_number(arg1) ||
      !is_number(arg2)) {
...
void obj_command_interpreter(obj_data *obj, char *argument) {
  ...
  if (*obj_cmd_info[cmd].command == '\n')
    ...
    ((*obj_cmd_info[cmd].command_pointer)(obj, line, cmd,
                                          obj_cmd_info[cmd].subcmd));
}
