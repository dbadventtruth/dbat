...
void huge_update(uint64_t heartPulse, double deltaTime) {
    ...
    for (const auto& r : objectSubscriptions.all("hugeKiAttacks")) {
        ...
        if (GET_OBJ_VNUM(k) != 82 && GET_OBJ_VNUM(k) != 83) {
            ...
            /* Genki Dama Section */
            if (KITYPE(k) == 497) {
                if (TARGET(k)->getLocation() == k->getLocation()) {
                    ch = USER(k);
                ...
                } else if (TARGET(k)->getLocation() != k->getLocation()) {
                    ch = USER(k);
    ...
    for (const auto& r : objectSubscriptions.all("homingKiAttacks")) {
        ...
        if (GET_OBJ_VNUM(k) == 80) { // Tsuihidan
            ...
            if (k->getLocation() != vict->getLocation()) {
                ...
                k->setLocation(vict->getLocation());
                continue;
            } else {
                ...
                if (handle_parry(vict) < rand_number(1, 140)) {
                    ...
                    act("@C$n @wmanages to deflect the $p@w sending it flying away into the nearby surroundings!@n",
                        true, vict, k, nullptr, TO_ROOM);
                    if (vict->getLocationDamage() <= 95) {
                        vict->modLocationDamage(5);
                    }
                    extract_obj(k);
                    continue;
                }
...
parry_ki(double attperc, struct char_data *ch, struct char_data *vict, char sname[1000], int prob, int perc, int skill,
    ...
    if ((foundo == false || foundv == false) && !vict->getLocationRoomFlag(ROOM_SPACE)) {
        sprintf(buf,
        ...
        if (ch->getLocationDamage() <= 95) {
            ch->modLocationDamage(5);
        }
        area_explosion(ch);
        return;
    }
...
void dodge_ki(struct char_data *ch, struct char_data *vict, int type, int type2, int skill, int skill2) {
    if (type == 0 && !vict->getLocationRoomFlag(ROOM_SPACE)) {
...
static void spar_helper(struct char_data *ch, struct char_data *vict, int type, int64_t dmg) {
    ...
    if (ch->getLocationRoomFlag(ROOM_WORKOUT) || (ch->getLocationRoomFlag(ROOM_HBTC))) {
...
int can_kill(struct char_data *ch, struct char_data *vict, struct obj_data *obj, int num) {
    ...
    if (vict) {
        ...
        if (ch->getLocationRoomFlag(ROOM_PEACEFUL)) {
            ...
            return 0;
        } else if (MAJINIZED(ch) == ((vict)->getID())) {
            ...
            return 0;
        } else if (MAJINIZED(vict) == ((ch)->getID())) {
    ...
    if (vict) {
        ...
        if (is_sparring(ch) && is_sparring(vict) && (GET_SUPPRESS(vict) + vict->getCurHealth()) - dmg <= 0) {
            if (!IS_NPC(vict)) {
                ...
                for (auto rew : IterRef(vict->getContents())) {
                    rew->setLocation(vict->getLocation());
                    founded = 1;
                }
                if (founded == 1) {
        ...
        if (PLR_FLAGGED(vict, PLR_IMMORTAL) && !is_sparring(ch) && vict->getCurHealth() - dmg <= 0) {
            if (IN_ARENA(vict)) {
                ...
                vict->setCurHealth(1);
                look_at_location(vict->getLocation(), vict, 0);
                if (FIGHTING(vict)) {
                    stop_fighting(vict);
                }
                if (FIGHTING(ch)) {
...
void handle_death_msg(struct char_data *ch, struct char_data *vict, int type) {
    ...
    if (type == 0) {
        if (vict->getLocationEnvironment(ENV_WATER) < 100.0 && tile != SECT_WATER_SWIM &&
            tile != SECT_WATER_NOSWIM && !vict->getLocationRoomFlag(ROOM_SPACE) &&
            tile != SECT_FLYING) {
            switch (rand_number(1, 5)) {
                case 1:
        ...
        } else if (vict->getLocationRoomFlag(ROOM_SPACE)) {
            switch (rand_number(1, 5)) {
                case 1:
