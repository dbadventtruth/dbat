...
ACMD(do_oasis_hedit) {
  ...
  if (HEDITS == TRUE) {
    send_to_char(ch,
                 "Sorry, only one person can edit help files at a time.\r\n");
    return;
  }

  if (GET_ADMLEVEL(ch) < 4 && (!strcasecmp("Tepsih", GET_NAME(ch)) &&
                               !strcasecmp("Rogoshen", GET_NAME(ch)))) {
    send_to_char(
        ch, "Sorry you are incapable of editing help files at this time.\r\n");
    return;
  }
  ...
  if (!strcasecmp("save", arg)) {
    mudlog(CMP, MAX(ADMLVL_BUILDER, GET_INVIS_LEV(ch)), TRUE,
           "OLC: %s saves help files.", GET_NAME(ch));
  ...
  mudlog(CMP, ADMLVL_IMMORT, TRUE, "OLC: %s starts editing help files.",
         GET_NAME(ch));
}
...
static void hedit_save_to_disk(struct descriptor_data *d) {
  ...
  if (!(fp = fopen(index_name, "w"))) {
    mud_log("SYSERR: Could not write help index file");
    return;
  }
...
static void hedit_disp_menu(struct descriptor_data *d) {
  write_to_output(d,
                  ...
                  "@gQ@n) Quit\r\n"
                  "Enter choice : ",
                  OLC_HELP(d)->keywords, OLC_HELP(d)->entry,
...
void hedit_parse(struct descriptor_data *d, char *arg) {
    ...
    case 'Y':
      if (OLC_HELP(d)->keywords == NULL) {
        ...
        *buf = '\0';
        ...
        send_to_imm("@gHedit@D: @w%s@G has just edited and saved, @Y%s@G.@n",
                    d->character->name, buf);
  ...
  case HEDIT_ENTRY:
    ...
    mudlog(TRUE, ADMLVL_BUILDER, BRF,
...
ACMD(do_helpcheck) {
  ...
  for (i = 1; *(complete_cmd_info[i].command) != '\n'; i++) {
    if (complete_cmd_info[i].command_pointer != do_action &&
      ...
      if (search_help((char *)(complete_cmd_info[i].command), ADMLVL_IMPL) ==
          NOWHERE) {
        /*if (search_help(complete_cmd_info[i].command, ADMLVL_IMPL) ==
        NOWHERE) {*/
        nlen =
            snprintf(buf + len, sizeof(buf) - len, "%-20.20s%s",
