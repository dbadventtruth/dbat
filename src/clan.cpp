...
bool isClan(const char *name) { return (clanGet(name) != NULL); }
...
void clanINFOW(char *name, struct char_data *ch) {
  ...
  if (S == NULL || IS_NPC(ch))
    ...
    write_to_output(
        ch->desc,
        "@rYou are limited to 1000 characters for the clan info.@n\r\n");
...
bool clanRANKD(const char *name, struct char_data *ch, struct char_data *vict) {
  ...
  if (S == NULL || IS_NPC(ch)) {
    ...
    if (GET_CRANK(vict) == 0 &&
        !clanMemberFromList(GET_IDNUM(vict), S->moderators)) {
      send_to_char(ch, "Member@n\r\n");
    } else if (GET_CRANK(vict) == 1 &&
               !clanMemberFromList(GET_IDNUM(vict), S->moderators)) {
      send_to_char(ch, "%s@n\r\n", S->midrank);
    } else if (GET_CRANK(vict) == 2 &&
               !clanMemberFromList(GET_IDNUM(vict), S->moderators)) {
      send_to_char(ch, "%s@n\r\n", S->highrank);
    } else {
...
bool clanBSET(const char *name, struct char_data *ch) {
  ...
  if (S->bany > 0) {
    S->bany = 0;
    send_to_char(
        ch, "The clan bank will now only be accessible from its room.\r\n");
    clanSave(S, clanFilename(S));
    return TRUE;
  } else {
    S->bany = 1;
...
void listClanInfo(const char *name, struct char_data *ch) {
  ...
  send_to_char(
      ...
      (S->open_leave == FALSE) ? "Players must be expelled to leave this clan"
                               : "Players may leave this clan as they please",
