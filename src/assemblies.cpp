...
bool assemblyAddComponent(long lVnum, long lComponentVnum, bool bExtract,
                          bool bInRoom) {
  ASSEMBLY *pAssembly = NULL;
  ...
  if (g_lNumAssemblies > 1) {
    ...
    if (lIndex < g_lNumAssemblies - 1) {
      memmove(pNewAssemblyTable + lIndex, g_pAssemblyTable + lIndex + 1,
              (g_lNumAssemblies - lIndex - 1) * sizeof(ASSEMBLY));
    }
  }
...
bool assemblyRemoveComponent(long lVnum, long lComponentVnum) {
  ...
  if (pAssembly->pComponents != NULL && pAssembly->lNumComponents > 1) {
    ...
    if (lIndex > 0)
      memmove(pNewComponents, pAssembly->pComponents,
              lIndex * sizeof(COMPONENT));
