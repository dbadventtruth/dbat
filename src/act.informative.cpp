...
ACMD(do_evolve) {
  ...
  int64_t plcost = GET_LEVEL(ch), stcost = GET_LEVEL(ch),
          kicost = GET_LEVEL(ch);
  ...
  if (!*arg) {
    ...
    if (plcost > molt_threshold(ch)) {
      send_to_char(ch, "You need a few more evolution levels before you can "
                       "start upgrading powerlevel.\r\n");
      return;
      ...
      send_to_char(
          ch,
          "Your body evolves to make better use of the way it is now, and you "
          "feel that your body has strengthened. @D[@RPL@D: @Y+%s@D]@n\r\n",
          add_commas(plgain));
    }
    ...
    if (kicost > molt_threshold(ch)) {
      ...
      send_to_char(
          ch,
          "Your body evolves to make better use of the way it is now, and you "
          "feel that your spirit has strengthened. @D[@CKi@D: @Y+%s@D]@n\r\n",
          add_commas(kigain));
    }
  } else if (!strcasecmp(arg, "stamina") || !strcasecmp(arg, "st")) {
    if (stcost > molt_threshold(ch)) {
      send_to_char(ch, "You need a few more evolution levels before you can "
                       "start upgrading stamina.\r\n");
      return;
      ...
      send_to_char(
          ch,
          "Your body evolves to make better use of the way it is now, and you "
          "feel that your body has more stamina. @D[@GST@D: @Y+%s@D]@n\r\n",
          add_commas(stgain));
    }
  }
}
...
ACMD(do_kyodaika) {
  ...
  if (!char_condition_has(ch, "kyodaika")) {
    ...
    act("@GYou growl as your body shrinks to its normal size!@n", TRUE, ch, 0,
        0, TO_CHAR);
...
ACMD(do_draw) {
  ...
  if (GET_OBJ_VNUM(SITS(ch)) < 604 || GET_OBJ_VNUM(SITS(ch)) > 607) {
    send_to_char(ch,
                 "You need to be sitting at an official table to play.\r\n");
    return;
  }
...
ACMD(do_hand) {
  ...
  if (!strcasecmp("look", arg)) {
    ...
    if (count == 0) {
      ...
      act("You have more than seven cards in your hand.", TRUE, ch, 0, 0,
          TO_CHAR);
...
ACMD(do_play) {
  ...
  if (obj2 == NULL) {
    send_to_char(
        ch, "Your table is missing. Inform an immortal of this problem.\r\n");
    return;
  }
...
ACMD(do_showoff) {
  ...
  *arg2 = '\0';
  ...
  if (!(obj = get_obj_in_list_vis(ch, arg, NULL, inv_for_char(ch)))) {
    ...
    act("@WYou hold up $p@W for @C$N@W to see:@n", TRUE, ch, obj, vict,
        TO_CHAR);
    act("@C$n@W holds up $p@W for you to see:@n", TRUE, ch, obj, vict, TO_VICT);
    act("@C$n@W holds up $p@W for @c$N@W to see.@n", TRUE, ch, obj, vict,
        TO_NOTVICT);
...
ACMD(do_status) {
  ...
  const char *forget_level[7] = {"@GRemembered Well@n",
  ...
  if (!*arg) {
    ...
    if (PLR_FLAGGED(ch, PLR_HEAD)) {
      send_to_char(
          ch, "            @D[@cHead        @D: @GHave.          @D]@n\r\n");
    }
    if (!PLR_FLAGGED(ch, PLR_HEAD)) {
      send_to_char(
          ch, "            @D[@cHead        @D: @rMissing.         @D]@n\r\n");
    }
    if (GET_LIMBCOND(ch, 1) >= 50 && !PLR_FLAGGED(ch, PLR_CRARM)) {
      ...
      send_to_char(
          ch, "            @D[@cRight Arm   @D: @rMissing.         @D]@n\r\n");
    }
    ...
    if (GET_LIMBCOND(ch, 3) >= 50 && !PLR_FLAGGED(ch, PLR_CLARM)) {
      ...
      send_to_char(
          ch, "            @D[@cRight Leg   @D: @rMissing.         @D]@n\r\n");
    }
    if (GET_LIMBCOND(ch, 4) >= 50 && !PLR_FLAGGED(ch, PLR_CLLEG)) {
      ...
      send_to_char(
          ch, "            @D[@cLeft Leg    @D: @rMissing.         @D]@n\r\n");
    }

    if ((IS_SAIYAN(ch) || IS_HALFBREED(ch)) && PLR_FLAGGED(ch, PLR_STAIL)) {
      send_to_char(
          ch, "            @D[@cTail        @D: @GHave.            @D]@n\r\n");
    }
    if ((IS_SAIYAN(ch) || IS_HALFBREED(ch)) && !PLR_FLAGGED(ch, PLR_STAIL)) {
      send_to_char(
          ch, "            @D[@cTail        @D: @rMissing.         @D]@n\r\n");
    }
    ...
    if (NEWSUPDATE > GET_LPLAY(ch)) {
      send_to_char(
          ch, "         Check the 'news', it has been updated recently.\r\n");
    }
    if (has_mail(GET_IDNUM(ch))) {
      send_to_char(ch,
                   "         Check your mail at the nearest postmaster.\r\n");
    }
    ...
    if (GET_PREFERENCE(ch) == 0) {
      ...
      send_to_char(
          ch, "         You preferred a ki dominate form of fighting.\r\n");
    } else if (GET_PREFERENCE(ch) == PREFERENCE_WEAPON) {
      send_to_char(
          ch, "         You preferred a weapon dominate form of fighting.\r\n");
    } else if (GET_PREFERENCE(ch) == PREFERENCE_H2H) {
      send_to_char(
          ch, "         You preferred a body dominate form of fighting.\r\n");
    } else if (GET_PREFERENCE(ch) == PREFERENCE_THROWING) {
      send_to_char(
          ch,
          "         You preferred a throwing dominate form of fighting.\r\n");
    }
    if (GET_DISTFEA(ch) == DISTFEA_HAIR && !IS_DEMON(ch) && !IS_MAJIN(ch) &&
        !IS_ICER(ch) && !IS_NAMEK(ch)) {
      send_to_char(ch,
                   "         Your hair is your most distinctive feature.\r\n");
    } else if (GET_DISTFEA(ch) == DISTFEA_HAIR && IS_DEMON(ch)) {
      send_to_char(
          ch, "         Your horns are your most distinctive feature.\r\n");
    } else if (GET_DISTFEA(ch) == DISTFEA_HAIR && IS_MAJIN(ch)) {
      send_to_char(
          ch, "         Your forelock is your most distinctive feature.\r\n");
    } else if (GET_DISTFEA(ch) == DISTFEA_HAIR && IS_ICER(ch)) {
      send_to_char(
          ch, "         Your horns are your most distinctive feature.\r\n");
    } else if (GET_DISTFEA(ch) == DISTFEA_HAIR && IS_NAMEK(ch)) {
      send_to_char(
          ch, "         Your antennae are your most distinctive feature.\r\n");
    }

    if (GET_DISTFEA(ch) == DISTFEA_SKIN) {
      send_to_char(ch,
                   "         Your skin is your most distinctive feature.\r\n");
    }
    if (GET_DISTFEA(ch) == DISTFEA_HEIGHT) {
      send_to_char(
          ch, "         Your height is your most distinctive feature.\r\n");
    }
    if (GET_DISTFEA(ch) == DISTFEA_WEIGHT) {
      send_to_char(
          ch, "         Your weight is your most distinctive feature.\r\n");
    }
    ...
    if (AFF_FLAGGED(ch, AFF_SANCTUARY)) {
      send_to_char(ch,
                   "         You are surrounded by a barrier @D(@Y%s@D)@n\r\n",
    ...
    if (GET_SUPPRESS(ch) > 0) {
      send_to_char(ch,
                   "         You are suppressing current PL to %" I64T ".\r\n",
                   GET_SUPPRESS(ch));
    }
    if (IS_MAJIN(ch)) {
    ...
    if (GET_LEVEL(ch) < 100) {
      if ((IS_ANDROID(ch) && PLR_FLAGGED(ch, PLR_ABSORB)) ||
          (!IS_ANDROID(ch) && !IS_BIO(ch) && !IS_MAJIN(ch))) {
        send_to_char(ch, "         @R%s@n to SC a stat this level.\r\n",
                     add_commas(calc_soft_cap(ch)));
      } else {
        send_to_char(
            ch, "         @R%s@n in PL/KI/ST combined to SC this level.\r\n",
            add_commas(calc_soft_cap(ch)));
      }
    } else {
      send_to_char(ch,
    ...
    if (GET_RELAXCOUNT(ch) > 464) {
      ...
      send_to_char(ch, "You are too at ease to train hard like you should. Get "
                       "out of the house more often.\r\n");
    } else if (GET_RELAXCOUNT(ch) > 116) {
      send_to_char(ch, "You are a bit at ease and your training suffers. Get "
                       "out of the house more often.\r\n");
    }
    ...
    if (ch->affected) {
      ...
      for (aff = ch->affected; aff; aff = aff->next) {
        if (!strcasecmp(skill_name(aff->type), "runic") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "Your Kenaz rune is still in effect! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "punch") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "Your Algiz rune is still in effect! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "knee") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "Your Oagaz rune is still in effect! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "slam") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "Your Wunjo rune is still in effect! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "heeldrop") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "Your Purisaz rune is still in effect! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "special beam cannon") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "Your Laguz rune is still in effect! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "might") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "Your muscles are pumped! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "flex") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You are more agile right now! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "bless") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You have been blessed! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "curse") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You have been cursed! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "healing glow") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch,
                       "You have a healing glow enveloping your body! (%2d Mud "
                       "Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "genius") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You are smarter right now! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "enlighten") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You are wiser right now! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "yoikominminken") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You have been lulled to sleep! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "solar flare") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You have been blinded! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "spirit control") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch, "You have full control of your spirit! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "!UNUSED!") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch,
              "You feel poison burning through your blood! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "tough skin") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch,
                       "You have toughened skin right now! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "poison") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "You have been poisoned! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "warp pool") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(ch, "Weakened State! (%2d Mud Hours)\r\n",
                       aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "dark metamorphosis") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch,
              "Your Dark Metamorphosis is still in effect. (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
        if (!strcasecmp(skill_name(aff->type), "hayasa") &&
            aff->type != lasttype) {
          lasttype = aff->type;
          send_to_char(
              ch,
              "Your body has been infused to move faster! (%2d Mud Hours)\r\n",
              aff->duration + 1);
        }
      }
    }
    ...
    if (AFF_FLAGGED(ch, AFF_DETECT_INVIS))
      send_to_char(
          ch, "You are sensitive to the presence of invisible things.\r\n");
    ...
    if (AFF_FLAGGED(ch, AFF_DETECT_MAGIC))
      send_to_char(ch,
                   "You are sensitive to the magical nature of things.\r\n");
    ...
    if (PRF_FLAGGED(ch, PRF_NOGIVE))
      send_to_char(
          ch, "You are not accepting items being handed to you right now.\r\n");
    ...
    send_to_char(ch, "@D<@b----------------------------------------------------"
                     "----------@D>@n\r\n");
    send_to_char(ch,
                 "To view your bonus/negative traits enter: status traits\r\n");
  } else if (!strcasecmp(arg, "traits")) {
    bonus_status(ch);
  } else {
    send_to_char(ch, "The only argument status takes is 'traits'. If you just "
        ...
        list_bonuses[] =
            {
                ...
                "    ", /* Bonus 2 */
                "Die Hard    - Life Force's PL regen doubled, but cost is the "
                "same ", /* Bonus 3 */
                "Brawler     - Physical attacks do 20% more damage             "
                "    ", /* Bonus 4 */
                "Destroyer   - Damaged Rooms act as regen rooms for you        "
                "    ", /* Bonus 5 */
                "Hard Worker - Physical activity bonuses + drains less stamina "
                "    ", /* Bonus 6 */
                "Healer      - Heal/First-aid/Vigor/Repair restore +10%        "
                "    ", /* Bonus 7 */
                "Loyal       - +20% Experience When Grouped As Follower        "
                "    ", /* Bonus 8 */
                "Brawny      - Strength gains +2 every 10 levels, Train STR + "
                "75%  ", /* Bonus 9 */
                "Scholarly   - Intelligence gains +2 every 10 levels, Train "
                "INT + 75%", /* Bonus 10 */
                "Sage        - Wisdom gains +2 every 10 levels, Train WIS + "
                "75%    ", /* Bonus 11 */
                "Agile       - Agility gains +2 every 10 levels, Train AGL + "
                "75%   ", /* Bonus 12 */
                "Quick       - Speed gains +2 every 10 levels, Train SPD + 75% "
                "    ", /* Bonus 13 */
                "Sturdy      - Constitution +2 every 10 levels, Train CON + "
                "75%    ", /* Bonus 14 */
                ...
                "    ", /* Bonus 18 */
                "Healthy     - 40% chance to recover from ill effects when "
                "sleeping", /* Bonus  19 */
                "Insomniac   - Can't Sleep. Immune to yoikominminken and "
                "paralysis ", /* Bonus  20 */
                "Evasive     - +15% to dodge rolls                             "
                "    ", /* Bonus  21 */
                "The Wall    - +20% chance to block                            "
                "    ", /* Bonus  22 */
                "Accurate    - +20% chance to hit physical, +10% to hit with "
                "ki     ", /* Bonus  23 */
                "Energy Leech- -2% ki damage received for every 5 character "
                "levels,\n                  @cas long as you can take that ki "
                "to your charge pool.@D        ", /* Bonus  24*/
                "Good Memory - +2 Skill Slots initially, +1 every 20 levels "
                "after  ", /* Bonus 25 */
                "Soft Touch  - Half damage for all hit locations               "
                "    ", /* Neg 26 */
                "Late Sleeper- Can only wake automatically. 33% every hour if "
                "maxed", /* Neg 27 */
                ...
                "    ", /* Neg 34 */
                "Energy Int. - +2% ki damage received for every 5 character "
                "levels,\n                  @rif you have ki charged you have "
                "10% chance to lose   \n                  it and to take 1/4th "
                "damage equal to it.@D                    ", /* Neg 35 */
                ...
                "    ", /* Neg 49 */
                "Loner       - Can't Group with anyone, +5% train and +10% "
                "Phys    ", /* Neg 50 */
...
ACMD(do_weather) {
  const char *sky_look[] = {"cloudless", "cloudy", "rainy",
  ...
  if (OUTSIDE(ch)) {
    ...
    if (ADM_FLAGGED(ch, ADM_KNOWWEATHER))
      send_to_char(ch, "Pressure: %d (change: %d), Sky: %d (%s)\r\n",
                   weather_info.pressure, weather_info.change, weather_info.sky,
                   sky_look[weather_info.sky]);
  } else
...
ACMD(do_levels) {
  ...
  for (i = 1; i < 101; i++) {
    if (i == 100)
      ...
      nlen = snprintf(
          buf + len, sizeof(buf) - len, "[%2" SZT "] %8s-%-8s : \r\n", i,
...
ACMD(do_consider) {
  ...
  if (diff <= -10)
    ...
    send_to_char(ch, "Maybe if they are allergic to you, otherwise your last "
                     "words will be 'Oh shit.'\r\n");
  else
