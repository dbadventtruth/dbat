...
ACMD(do_action) {
  ...
  if (!vict) {
    if (action->char_obj_found) {
      ...
      if (!targ)
        targ = get_obj_in_list_vis(ch, arg, NULL, room_contents_get(char_room_get(ch)));
      if (targ) {
  ...
  if (GET_POS(vict) < action->min_victim_position)
    ...
    if (*part) {
      act(action->char_body_found, 0, ch, (struct obj_data *)part, vict,
          TO_CHAR | TO_SLEEP);
      act(action->others_body_found, action->hide, ch, (struct obj_data *)part,
          vict, TO_NOTVICT);
      act(action->vict_body_found, action->hide, ch, (struct obj_data *)part,
          vict, TO_VICT);
    } else {
...
ACMD(do_gmote) {
  ...
  if ((act_nr = find_action(cmd)) < 0) {
    snprintf(buf, sizeof(buf), "@D[@BOOC@D: @g%s %s@n@D]",
             GET_ADMLEVEL(ch) < 1 ? ch->desc->user : GET_NAME(ch), argument);
