...
void broken_update() {
  ...
  int rand_gravity[14] = {0,   10,  20,  30,  40,   50,   100,
  ...
  for (k = object_list; k; k = k->next) {
    ...
    health = GET_OBJ_VAL(
        k, VAL_ALL_HEALTH); // Indicated the health of the object in question
    ...
    if (GET_OBJ_VNUM(k) == 3034) { /* ATM */
      if (health <= 10) {
        send_to_room(
            room, "@RThe ATM machine shoots smoking bills from its money slot. "
