...
void do_dg_cast(void *go, struct script_data *sc, trig_data *trig, int type,
  ...
  if ((spellnum < 0) || (spellnum >= SKILL_TABLE_SIZE ||
                         (!IS_SET(skill_type(spellnum), SKTYPE_SPELL)))) {
...
void do_dg_affect(void *go, struct script_data *sc, trig_data *trig,
  ...
  if (charname == NULL || !*charname || property == NULL || !*property ||
    ...
    script_log("Trigger: %s, VNum %d. dg_affect usage: <target> <property> "
               "<value> <duration>",
               GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
    return;
  }
  ...
  if (duration <= 0) {
    script_log("Trigger: %s, VNum %d. dg_affect: need positive duration!",
               GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
    script_log("Line was: dg_affect %s %s %s %s (%d)", charname, property,
               value_p, duration_p, duration);
    return;
  }
