...
ACMD(do_zanzoken) {
  ...
  if (prob < perc) {
    send_to_char(ch, "You focus your ki in preparation of a zanzoken but mess "
                     "up and waste your ki!\r\n");
...
ACMD(do_block) {
  ...
  if (BLOCKS(ch)) {
    ...
    act("@wYou start blocking @c$N's@w escape.@n", TRUE, ch, 0, BLOCKS(ch),
        TO_CHAR);
    act("@C$n@w starts blocking your escape.@n", TRUE, ch, 0, BLOCKS(ch),
        TO_VICT);
    act("@C$n@w starts blocking @c$N's@w escape.@n", TRUE, ch, 0, BLOCKS(ch),
        TO_NOTVICT);
    return;
  } else {
    BLOCKS(ch) = vict;
...
ACMD(do_solar) {
  ...
  if (prob < perc) {
    act("@WYou raise both your hands to either side of your face, while "
        "closing your eyes, and shout '@YSolar Flare@W' but nothing happens!@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@C$n@W raises both $s hands to either side of $s face, while closing "
        "$s eyes, and shouts '@YSolar Flare@W' but nothing happens!@n",
        TRUE, ch, 0, 0, TO_ROOM);
    decCurKI(ch, cost);
    WAIT_STATE(ch, PULSE_3SEC);
  ...
  act("@WYou raise both your hands to either side of your face, while closing "
      "your eyes, and shout '@YSolar Flare@W' as a blinding light fills the "
      "area!@n",
      TRUE, ch, 0, 0, TO_CHAR);
  act("@C$n@W raises both $s hands to either side of $s face, while closing $s "
      "eyes, and shouts '@YSolar Flare@W' as a blinding light fills the "
      "area!@n",
      TRUE, ch, 0, 0, TO_ROOM);
...
ACMD(do_heal) {
  ...
  if (GET_SUPPRESS(vict) > 0 &&
      GET_HIT(vict) >= ((getMaxPL(vict) / 100) * GET_SUPPRESS(vict))) {
    send_to_char(ch, "They are already at full health.\r\n");
    return;
  }
  ...
  if (prob < perc) {
    if (vict != ch) {
      act("@WYou place your hands near @c$N@W, but fail to concentrate enough "
          "to heal them!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@W places $s hands near you, but nothing happens!@n", TRUE, ch,
          0, vict, TO_VICT);
      act("@C$n@W places $s hands near @c$N@W, but nothing happens.", TRUE, ch,
          0, vict, TO_NOTVICT);
      decCurKI(ch, cost);
    ...
    if (vict == ch) {
      act("@WYou place your hands on your body, but fail to concentrate to "
          "heal yourself!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@W places $s hands on $s body, but nothing happens.", TRUE, ch,
          0, vict, TO_NOTVICT);
      decCurKI(ch, cost);
      improve_skill(ch, SKILL_HEAL, 0);
      WAIT_STATE(ch, PULSE_2SEC);
  ...
  if (vict != ch) {
    ...
    act("@WYou place your hands near @c$N@W and an orange glow surrounds $M!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@W places $s hands near you and an orange glow surrounds you!@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@C$n@W places $s hands near @c$N@W and an orange glow surrounds $M.",
        TRUE, ch, 0, vict, TO_NOTVICT);
    decCurKI(ch, cost);
    ...
    if (!PLR_FLAGGED(vict, PLR_STAIL) &&
        (IS_SAIYAN(vict) || IS_HALFBREED(vict)) && !IS_NPC(vict)) {
    ...
    if (vict->master == ch || ch->master == vict ||
      ...
      if (IS_NAIL(ch) && IS_NAMEK(ch) &&
        ...
        char_stat_mod(ch, "experience",
                      level_exp(ch, GET_LEVEL(ch) + 1) * 0.005);
      }
    }
  ...
  if (vict == ch) {
    ...
    act("@WYou place your hands on your body and an orange glow surrounds "
        "you!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@W places $s hands on $s body and an orange glow surrounds $m.",
        TRUE, ch, 0, vict, TO_NOTVICT);
    decCurKI(ch, cost);
    incCurHealth(vict, heal);
...
ACMD(do_barrier) {
  ...
  if (AFF_FLAGGED(ch, AFF_SANCTUARY) && !strcasecmp("release", arg)) {
    act("@BYou dispel your barrier, releasing its energy.@n", TRUE, ch, 0, 0,
        TO_CHAR);
  ...
  if (size > prob) {
    send_to_char(ch, "You can not create a barrier that is stronger than your "
                     "skill in barrier.\r\n");
    return;
  } else if (size < 1) {
    ...
    send_to_char(ch,
                 "You can't control a barrier with more than 75 percent!\r\n");
    return;
  } else if (GET_CHARGE(ch) < cost) {
    send_to_char(ch, "You do not have enough ki charged up!\r\n");
    return;
  } else if (prob < perc) {
    act("@BYou shout as you form a barrier of ki around your body, but you "
        "imbalance it and it explodes outward!@n",
        TRUE, ch, 0, 0, TO_CHAR);
    ...
    if (GET_SKILL(ch, SKILL_BARRIER)) {
      act("@BYou shout as you form a barrier of ki around your body!@n", TRUE,
          ch, 0, 0, TO_CHAR);
      act("@B$n@B shouts as $e forms a barrier of ki around $s body!@n", TRUE,
          ch, 0, 0, TO_ROOM);
    } else {
      act("@BYou shout as you form a barrier of ki and raging waters around "
          "your body!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@B$n@B shouts as $e forms a barrier of ki and raging waters around "
          "$s body!@n",
          TRUE, ch, 0, 0, TO_ROOM);
    }
...
ACMD(do_instant) {
  ...
  if (!know_skill(ch, SKILL_INSTANTT)) {
    ...
    return;
  } else if (room_flagged(char_room_get(ch), ROOM_RHELL) ||
             room_flagged(char_room_get(ch), ROOM_AL) ||
             room_flagged(char_room_get(ch), ROOM_HELL)) {
    send_to_char(ch, "You can not leave where you are at!\r\n");
    return;
  } else if (!*arg) {
    send_to_char(ch, "Who or where do you want to instant transmission to? "
                     "[target | planet-(planet name)]\r\n");
    send_to_char(ch,
                 "Example: instant goku\nExample 2: instant planet-earth\r\n");
    return;
  }
  ...
  if (tar != NULL) {
    if (tar == ch) {
      ...
      send_to_char(ch, "You've never sensed them up close so you do not have a "
                       "good bearing on their ki signal.\r\n");
      return;
      ...
      return;
               ...
               room_flagged(char_room_get(tar), ROOM_AL)) {
      send_to_char(ch, "They are dead and can't be reached.\r\n");
      return;
    } else if (!room_flagged(char_room_get(ch), ROOM_RHELL) &&
               room_flagged(char_room_get(tar), ROOM_RHELL)) {
      send_to_char(ch, "They are dead and can't be reached.\r\n");
      return;
    ...
    decCurKI(ch, cost);
    act("@wPlacing two fingers on your forehead you close your eyes and "
        "concentrate. Accelerating to such a speed that you move through the "
        "molecules of the universe faster than the speed of light. You stop as "
        "you arrive at $N@w!@n",
        TRUE, ch, 0, tar, TO_CHAR);
    act("@w$n@w appears in an instant out of nowhere right next to you!@n",
        TRUE, ch, 0, tar, TO_VICT);
    act("@w$n@w places two fingers on $s forehead and disappears in an "
        "instant!@n",
    ...
    decCurKI(ch, cost);
    act("@wPlacing two fingers on your forehead you close your eyes and "
        "concentrate. Accelerating to such a speed that you move faster than "
        "light and arrive almost instantly at your destination. Having located "
        "the planet by its collective population's ki.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@w$n@w places two fingers on $s forehead and disappears in an "
        "instant!@n",
...
ACMD(do_transform) {
  ...
  // Moved this further down. No need to parse the entry if we quit early.
  // -Volund
  two_arguments(argument, arg, arg2);
  ...
  if (!strcasecmp("revert", arg)) {
    ...
    get_race(ch->race)->echoRevert(
        ch, get_race(ch->race)->flagToTier(cur_form.flag));
  ...
  if (!npc && (getBasePL(ch)) < trans_req(ch, to_tier)) {
    send_to_char(
        ch, "You are not strong enough to handle that transformation!\r\n");
    return;
  }
  ...
  // The stats are applied automatically in the new system just by having the
  // flag.
  ...
  if (get_race(ch->race)->raceHasNoisyTransformations()) {
    ...
    if (zone) {
      send_to_zone(
          "An explosion of power ripples through the surrounding area!\r\n",
          zone);
    };
  }

  send_to_sense(0, "You sense a nearby power grow unbelievably!", ch);
  sprintf(buf3, "@D[@GBlip@D]@r Transformed Powerlevel@D: [@Y%s@D]",
