...
void ping_ship(int vnum, int vnum2) {
  ...
  if (found == TRUE) {
    send_to_room(
        obj_room_get(controls),
        "@D[@RALERT@D: @YAn unknown radar signal has been detected!@D]@n");
  }
}
...
char *getmapchar(struct room_data *room, struct char_data *ch,
                 struct room_data *start, int vnum) {
  ...
  if (room == room_by_id(GET_RADAR1(ch)) ||
      room == room_by_id(GET_RADAR2(ch)) ||
      room == room_by_id(GET_RADAR3(ch))) {
    if (there) {
...
void printmap(struct room_data *room, struct char_data *ch, int type,
              int vnum) {
  int x = 0, lasty = -1;
  int y = 0;
  int sightradius;
  ...
  if (type == 0) {
    send_to_char(ch, "@b_______________________________________________________"
                     "_______________@n\n");
  }
