...
static void harvest_plant(struct char_data *ch, struct obj_data *plant) {
  ...
  if (skill >= 100) {
    ...
    reward += 8;
    send_to_char(ch, " and your great skill has also helped the plant be more "
                     "bountiful yet!@n\r\n");
  } else if (skill >= 80) {
    reward += 5;
    send_to_char(ch, " and your good skill has also helped the plant be more "
                     "bountiful yet!@n\r\n");
  } else if (skill >= 50) {
    reward += 3;
    send_to_char(ch, " and your decent skill has also helped the plant be more "
                     "bountiful yet!@n\r\n");
  } else if (skill >= 40) {
    reward += 2;
    send_to_char(ch, " and your mastery of the basics of gardening has also "
                     "helped the plant be more bountiful yet!@n\r\n");
  } else if (skill >= 30) {
    reward += 1;
    send_to_char(ch, " and you somehow managed to make the plant slightly more "
                     "bountiful with what little you know!@n\r\n");
  } else {
    send_to_char(ch, ".@n\r\n");
  }
...
ACMD(do_garden) {
  ...
  } else if (!GET_SKILL(ch, SKILL_GARDENING) &&
             slot_count(ch) + 1 > GET_SLOTS(ch)) {
    send_to_char(ch, "You need additional skill slots to pick up the skill "
                     "linked with this.\r\n");
    return;
  }
  ...
  if (*arg) {
    if (!strcasecmp(arg, "collect")) {
      ...
      if (!(shovel = char_inventory_search_vnum(
                ch, 254, FALSE, SEARCH_GENUINE | SEARCH_WORKING))) {
      ...
      if (sect != SECT_FOREST && sect != SECT_FIELD && sect != SECT_MOUNTAIN &&
          sect != SECT_HILLS) {
      ...
      if (room_flagged(room, ROOM_FERTILE1)) {
        ...
        obj_to_char(soil, ch);
        act("@yYou sink your shovel into the soft ground and manage to dig up "
            "a pile of fertile soil!@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@w$n@y sinks $s shovel into the soft ground and manages to dig up "
            "a pile of fertile soil!@n",
            TRUE, ch, 0, 0, TO_ROOM);
  ...
  if (!*arg || !*arg2) {
    send_to_char(
        ch,
        "Syntax: garden (plant) ( water | harvest | dig | plant | pick )\r\n");
    send_to_char(
        ch,
        "Syntax: garden collect [Will collect soil from a room with soil.\r\n");
    return;
  }
  ...
  if (!strcasecmp(arg2, "plant")) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg, NULL, inv_for_room(room)))) {
  ...
  if ((getCurST(ch)) < cost) {
    ...
    if (!strcasecmp(arg2, "water")) {
      ...
      if (!(water =
                char_inventory_search_vnum(ch, 251, FALSE, SEARCH_GENUINE))) {
        send_to_char(
            ch,
            "You need a bottle of grow water in order to water the plant.\r\n");
        return;
      }

      if (GET_OBJ_VAL(obj, VAL_WATERLEVEL) >= 500) {
        send_to_char(ch, "You stop as you realize that the plant already has "
                         "enough water.\r\n");
        return;
        ...
        act("@GAs you go to water @g$p@G you end up sloppily wasting about "
            "half of it on the ground.@n",
            TRUE, ch, obj, 0, TO_CHAR);
        act("@g$n@G takes a bottle of grow water and sloshes some of it on "
            "@g$p@G.@n",
        ...
        act("@GYou calmly and expertly pour the grow water on @g$p@G.@n", TRUE,
            ch, obj, 0, TO_CHAR);
        act("@g$n@G calmly and expertly pours some grow water on @g$p@G.@n",
            TRUE, ch, obj, 0, TO_ROOM);
        decCurST(ch, cost);
      ...
      if (!(clippers = char_inventory_search_vnum(
                ch, 253, FALSE, SEARCH_GENUINE | SEARCH_WORKING))) {
        send_to_char(ch, "You need a pair of gardening clippers in order to "
                         "harvest the plant.\r\n");
        return;
      }
      if (can_harvest(obj) == FALSE) {
        send_to_char(ch, "You can not harvest that plant. Instead, Syntax: "
                         "garden (plant) (pick)\r\n");
        return;
      } else if (GET_OBJ_VAL(obj, VAL_WATERLEVEL) <= -10) {
        send_to_char(ch, "That plant is dead!\r\n");
        return;
      } else if (GET_OBJ_VAL(obj, VAL_MATURITY) <
                 GET_OBJ_VAL(obj, VAL_MAXMATURE)) {
        send_to_char(ch, "You stop as you realize that the plant isn't mature "
                         "enough to harvest.\r\n");
        return;
      } else if (skill < axion_dice(-5)) {
        act("@GAs you go to harvest @g$p@G you end up cutting it in half "
            "instead!@n",
            TRUE, ch, obj, 0, TO_CHAR);
        act("@g$n@G attempts to harvest @g$p@G with $s clippers, but "
            "accidently cuts the plant in half!@n",
            TRUE, ch, obj, 0, TO_ROOM);
        decCurST(ch, cost);
        extract_obj(obj);
        WAIT_STATE(ch, PULSE_3SEC);
        ...
        act("@GYou calmly and expertly harvest @g$p@G.@n", TRUE, ch, obj, 0,
            TO_CHAR);
        act("@g$n@G calmly and expertly harvests @g$p@G.@n", TRUE, ch, obj, 0,
            TO_ROOM);
        decCurST(ch, cost);
      ...
      if (!(soil =
                char_inventory_search_vnum(ch, 255, FALSE, SEARCH_GENUINE))) {
        send_to_char(ch, "You need a pile of soil in order to plant.\r\n");
        return;
      }

      if (check_saveroom_count(ch, NULL) > 7 &&
          room_flagged(room, ROOM_GARDEN1)) {
        send_to_char(ch, "This room already has all its planters full. Try "
                         "digging up some plants.\r\n");
        return;
      } else if (check_saveroom_count(ch, NULL) > 19 &&
                 room_flagged(room, ROOM_GARDEN2)) {
        send_to_char(ch, "This room already has all its planters full. Try "
                         "digging up some plants.\r\n");
        return;
      } else if (skill < axion_dice(-5)) {
        act("@GYou end up digging a hole too shallow to hold @g$p@G. Better "
            "try again.@n",
            TRUE, ch, obj, 0, TO_CHAR);
        act("@g$n@G digs a very shallow hole in one of the planters and then "
            "realizes @g$p@G won't fit in it.@n",
            TRUE, ch, obj, 0, TO_ROOM);
        decCurST(ch, cost);
        WAIT_STATE(ch, PULSE_3SEC);
        improve_skill(ch, SKILL_GARDENING, 0);
        ...
        act("@GYou dig a proper sized hole and plant @g$p@G in it.@n", TRUE, ch,
            obj, 0, TO_CHAR);
        act("@g$n@G digs a proper sized hole in a planter and plants @g$p@G in "
            "it.@n",
            TRUE, ch, obj, 0, TO_ROOM);
        obj_from_char(obj);
      ...
      if (!OBJ_FLAGGED(obj, ITEM_MATURE)) {
        send_to_char(ch, "You can't pick that type of plant. Syntax: garden "
                         "(plant) harvest\r\n");
        return;
      } else if (GET_OBJ_VAL(obj, VAL_MATURITY) <
                 GET_OBJ_VAL(obj, VAL_MAXMATURE)) {
        ...
        act("@GYou end up shredding @g$p@G with your clumsy unskilled hands.@n",
            TRUE, ch, obj, 0, TO_CHAR);
        act("@g$n@G grabs a hold of @g$p@G and shreds it in an attempt to pick "
            "it.@n",
            TRUE, ch, obj, 0, TO_ROOM);
        return;
      } else {
        act("@GYou grab a hold of @g$p@G and carefully pick it out of the "
            "soil.@n",
            TRUE, ch, obj, 0, TO_CHAR);
        act("@g$n@G grabs a hold of @g$p@G and carefully picks it out of the "
            "soil.@n",
            TRUE, ch, obj, 0, TO_ROOM);
        obj_from_room(obj);
        obj_to_char(obj, ch);
      ...
      send_to_char(ch, "Syntax: garden (plant) ( water | harvest | dig | plant "
                       "| pick )\r\n");
      send_to_char(ch, "Syntax: garden collect [Will collect soil from a room "
                       "with soil.\r\n");
      return;
    }
  }
...
ACMD(do_drink) {
  ...
  if (char_stat_get(ch, "hunger") <= 1 && char_stat_get(ch, "thirst") >= 2 &&
      !IS_NAMEK(ch) && GET_GENOME(ch, 0) != 3 && GET_GENOME(ch, 1) != 3) {
  ...
  if (!*arg && !IS_NPC(ch)) {
    ...
    case SECT_UNDERWATER:
      snprintf(buf, sizeof(buf),
               "$n takes a refreshing drink from the surrounding water.");
      act(buf, TRUE, ch, 0, 0, TO_ROOM);
      send_to_char(
          ch, "You take a refreshing drink from the surrounding water.\r\n");
      gain_condition(ch, THIRST, 1);
      if (GET_SKILL(ch, SKILL_WELLSPRING) &&
          (getCurKI(ch)) < GET_MAX_MANA(ch) && wasthirsty <= 30 &&
          subcmd != SCMD_SIP) {

        incCurKI(ch, ((GET_MAX_MANA(ch) * 0.005) +
                      (GET_WIS(ch) * rand_number(80, 100))) *
    ...
    default:
      if (!room_is_sunken(char_room_get(ch))) {
        ...
        send_to_char(
            ch, "You take a refreshing drink from the surrounding water.\r\n");
        gain_condition(ch, THIRST, 1);
        if (GET_SKILL(ch, SKILL_WELLSPRING) && !isFullKI(ch) &&
            wasthirsty <= 30 && subcmd != SCMD_SIP) {
          if (incCurKI(ch, ((GET_MAX_MANA(ch) * 0.005) +
                            (GET_WIS(ch) * rand_number(80, 100))) *
                               GET_SKILL(ch, SKILL_WELLSPRING)) ==
              getMaxKI(ch)) {
            send_to_char(ch, "You feel your ki return to full strength.\r\n");
  ...
  if (!(temp = get_obj_in_list_vis(ch, arg, NULL, inv_for_char(ch)))) {
  ...
  if ((GET_OBJ_TYPE(temp) != ITEM_DRINKCON) &&
    ...
    if (GET_OBJ_VNUM(temp) == 86 && on_ground != 1) {
      act("@wYou uncork the $p and tip it to your lips. Drinking it down you "
          "feel a warmth flow through your body and your ki returns to full "
          "strength .@n",
          TRUE, ch, temp, 0, TO_CHAR);
      act("@C$n@w uncorks the $p and tips it to $s lips. Drinking it down and "
          "then smiling.@n",
  ...
  if (GET_OBJ_VAL(temp, VAL_DRINKCON_HOWFULL) <= 0 &&
      (GET_OBJ_VAL(temp, VAL_DRINKCON_CAPACITY) >= 1)) {
  ...
  if (subcmd == SCMD_DRINK) {
    ...
    /* if (drink_aff[GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID)][DRUNK] > 0)
      amount = (25 - char_stat_get(ch, "thirst")) / drink_aff[GET_OBJ_VAL(temp,
    VAL_DRINKCON_LIQUID)][DRUNK]; else amount = rand_number(3, 10);*/
  ...
  gain_condition(ch, DRUNK,
                 drink_aff[GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID)][DRUNK] *
                     amount);
  gain_condition(ch, HUNGER,
                 drink_aff[GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID)][HUNGER] *
                     amount);
  ...
  if (GET_SKILL(ch, SKILL_WELLSPRING) && (getCurKI(ch)) < GET_MAX_MANA(ch) &&
      wasthirsty <= 30 && subcmd != SCMD_SIP) {
    if (GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID) == 0 ||
        GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID) == 14 ||
        GET_OBJ_VAL(temp, VAL_DRINKCON_LIQUID) == 15) {

      if (incCurKI(ch, ((GET_MAX_MANA(ch) * 0.005) +
                        (GET_WIS(ch) * rand_number(80, 100))) *
                           GET_SKILL(ch, SKILL_WELLSPRING)) == getMaxKI(ch)) {
        send_to_char(ch, "You feel your ki return to full strength.\r\n");
      } else {
        send_to_char(ch, "You feel your ki has rejuvenated.\r\n");
      }
  ...
  if (GET_OBJ_VAL(temp, VAL_DRINKCON_POISON) &&
      (!IS_MUTANT(ch) || (GET_GENOME(ch, 0) != 7 &&
                          GET_GENOME(ch, 1) != 7))) { /* The crap was poisoned
! */
...
ACMD(do_eat) {
  ...
  if (!(food = get_obj_in_list_vis(ch, arg, NULL, inv_for_char(ch)))) {
    send_to_char(ch, "You don't seem to have %s %s.\r\n", AN(arg), arg);
    return;
  }
  ...
  // amount that can be eaten - tasting only consumes 1 point, eating consumes
  // as much as possible VAL_FOOD_FOODVAL is the amount of nutrition remaining,
  // while value 1 is the max possible it once had.
  amount =
      MAX(1, MIN(foob, (subcmd == SCMD_EAT ? GET_OBJ_VAL(food, VAL_FOOD_FOODVAL)
                                           : 1)));
  ...
  if (subcmd != SCMD_TASTE) {
    int psbonus =
        (int)((double)(maxfval + GET_OBJ_VAL(food, VAL_FOOD_PSBONUS)) *
              percent_eaten);
    int expbonus = (int)((double)GET_OBJ_VAL(food, VAL_FOOD_EXPBONUS) *
                         ((GET_LEVEL(ch) * 0.4) + 1) * percent_eaten);
    gain_exp(ch, expbonus);
    char_stat_mod(ch, "practices", psbonus);
    send_to_char(ch,
                 "That was exceptionally delicious! @D[@mPS@D: @C+%d@D] "
                 "[@gEXP@D: @G+%s@D]@n\r\n",
                 psbonus, add_commas(expbonus));
    if (!GET_OBJ_VAL(food, VAL_FOOD_POISON) && GET_HIT(ch) < (getMaxPL(ch)) &&
        subcmd != SCMD_TASTE) {
...
static void majin_gain(struct char_data *ch, int type) {
  ...
  if (is_soft_cap(ch, 0)) {
    send_to_char(ch, "You can not gain anymore from candy consumption at your "
                     "current level.\r\n");
    return;
  }
  ...
  switch (type) {
  case -1:
    st = MIN(300000L, ((getBaseST(ch)) / 1200) +
                          rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    pl = MIN(300000L, ((getBasePL(ch)) / 1200) +
                          rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    ki = MIN(300000L, ((getBaseKI(ch)) / 1200) +
                          rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    break;
  case 0:
    st = MIN(500000L, ((getBaseST(ch)) / 1200) +
                          rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    pl = MIN(500000L, ((getBasePL(ch)) / 1200) +
                          rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    ki = MIN(500000L, ((getBaseKI(ch)) / 1200) +
                          rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    break;
  case 1:
    st = MIN(1200000L, ((getBaseST(ch)) / 1000) +
                           rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    pl = MIN(1200000L, ((getBasePL(ch)) / 1000) +
                           rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    ki = MIN(1200000L, ((getBaseKI(ch)) / 1000) +
                           rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    break;
  case 2:
    st = MIN(1500000L, ((getBaseST(ch)) / 900) +
                           rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    pl = MIN(1500000L, ((getBasePL(ch)) / 900) +
                           rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    ki = MIN(1500000L, ((getBaseKI(ch)) / 900) +
                           rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
    break;
  }
  ...
  send_to_char(ch,
               "@mYou feel stronger after consuming the candy @D[@RPL@W: @r%s "
               "@CKi@D: @c%s @GSt@D: @g%s@D]@m!@n\r\n",
