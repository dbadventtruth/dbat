...
ACMD(do_garden) {
    ...
    if (*arg) {
        if (!strcasecmp(arg, "collect")) {
            ...
            if (ch->getLocationRoomFlag(ROOM_FERTILE1)) {
                ...
                return;
            } else if (ch->getLocationRoomFlag(ROOM_FERTILE2)) {
                struct obj_data *soil = read_object(255, VIRTUAL);
                obj_to_char(soil, ch);
    ...
    if ((ch->getCurST()) < cost) {
        ...
        if (!strcasecmp(arg2, "water")) {
            ...
            } else if (check_saveroom_count(ch, nullptr) > 7 && ch->getLocationRoomFlag(ROOM_GARDEN1)) {
                send_to_char(ch, "This room already has all its planters full. Try digging up some plants.\r\n");
                return;
            } else if (check_saveroom_count(ch, nullptr) > 19 && ch->getLocationRoomFlag(ROOM_GARDEN2)) {
                send_to_char(ch, "This room already has all its planters full. Try digging up some plants.\r\n");
                return;
            } else if (skill < axion_dice(-5)) {
        ...
        if ((GET_OBJ_VNUM(obj) >= 19090 && GET_OBJ_VNUM(obj) <= 19099) || GET_OBJ_VNUM(obj) == 11) {
            ...
            if (!*arg2) {
                ...
                if (GET_OBJ_VNUM(obj) >= 18800 && GET_OBJ_VNUM(obj) <= 18899) {
                    ...
                    money = 65000;
                    while (count < 4) {
                        for(auto o : IterRef(world.at(rnum).getContents())) extract_obj(o);
                        count++;
                        rnum++;
                    }
...
int check_insidebag(struct obj_data *cont, double mult) {
    ...
    for (auto inside : IterRef(cont->getContents())) {
...
ACMD(do_deploy) {
    ...
    if (!*arg) {
        for (auto obj4 : IterRef(ch->getContents())) {
    ...
    if (capsule == false) {
        ...
        return;
    } else if (furniture == true && (!ch->getLocationRoomFlag(ROOM_HOUSE) || ch->getLocationRoomFlag(ROOM_SHIP))) {
        send_to_char(ch, "You can't deploy house furniture capsules here.\r\n");
        return;
    } else if (furniture == true &&
               (ch->getLocationRoomFlag(ROOM_GARDEN1) || ch->getLocationRoomFlag(ROOM_GARDEN2))) {
        send_to_char(ch, "You can't deploy house furniture capsules here.\r\n");
        return;
    ...
    while (giveup == false && cont == false) {
        obj3 = world.at(rnum).findObjectVnum(18801, false);
...
ACMD(do_auction) {
    ...
    if (!*arg1) {
        ...
        return;
    } else if (!(obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->getContents()))) {
...
ACMD(do_bid) {
    ...
    list += world.at(auct_room).getContents().size();
    masterList = list;
    list = 0;
...
ACMD(do_assemble) {
    ...
    for (auto tools : IterRef(ch->getContents())) {
    ...
    if ((lVnum = assemblyFindAssembly(arg2)) < 0 && (lVnum = assemblyGetAssemblyIndex(atoi(arg2))) < 0) {
        ...
        return;
    } else if (ch->getLocationRoomFlag(ROOM_SPACE)) {
    ...
    if (strcasecmp(arg2, "campfire")) {
        if (ch->getLocationRoomFlag(ROOM_SPACE) || ch->getLocationTileType() == SECT_WATER_NOSWIM || ch->getLocationEnvironment(ENV_WATER) >= 100.0) {
...
static void perform_put(struct char_data *ch, struct obj_data *obj,
    ...
    if (OBJ_FLAGGED(cont, ITEM_SHEATH)) {
        ...
        int count = 0, minus = 0;
        for (auto obj2 : IterRef(cont->getContents())) {
...
static bool validate_container(struct char_data *ch, struct obj_data *cont) {
    if (!cont) {
        send_to_char(ch, "You don't see that container here.\r\n");
        return false;
    }
...
static void perform_put_items(struct char_data *ch, struct obj_data *cont, const char *theobj, int howmany, bool all) {
    int found = 0;
...
ACMD(do_put) {
    ...
    if (!validate_container(ch, cont))
        return;

    if (obj_dotmode == FIND_INDIV) {
        perform_put_items(ch, cont, theobj, howmany, false);
    } else {
        perform_put_items(ch, cont, theobj, howmany, true);
    }
}
...
static void get_from_container(struct char_data *ch, struct obj_data *cont, char *arg, int mode, int howmany) {
    ...
    auto contents = IterRef(cont->getContents());

    if (obj_dotmode == FIND_INDIV) {
        auto it = contents.begin();
        while (it != contents.end() && howmany > 0) {
            auto obj = *it;
...
static void get_from_room(struct char_data *ch, char *arg, int howmany) {
    ...
    // Check if the player is trying to take something from a room's extra description
    ...
    if (dotmode == FIND_INDIV) {
        if (auto descword = find_exdesc_keywords(arg, ch->getRoom()->ex_description); descword != nullptr) {
            send_to_char(ch, "%s: you can't take that!\r\n", fname(descword));
            return;
        }

        auto objects = IterRef(ch->getLocationObjects());
        auto it = objects.begin();

        while (it != objects.end() && howmany > 0) {
            auto obj = *it;
            if (isname(arg, obj->name) && CAN_SEE_OBJ(ch, obj)) {
...
ACMD(do_get) {
    ...
    if (!*arg1)
        ...
        if (cont_dotmode == FIND_INDIV) {
            ...
            for (auto cont : IterRef(ch->getContents()))
...
static int perform_drop(struct char_data *ch, struct obj_data *obj,
    ...
    if (GET_OBJ_VNUM(obj) == 20 || GET_OBJ_VNUM(obj) == 21 || GET_OBJ_VNUM(obj) == 22 || GET_OBJ_VNUM(obj) == 23 ||
        ...
        if (ch->getLocationRoomFlag(ROOM_NOINSTANT)) {
            snprintf(buf, sizeof(buf), "You can't %s $p in this protected area!", sname);
            act(buf, false, ch, obj, nullptr, TO_CHAR);
            return (0);
        }
        if (ch->getLocationRoomFlag(ROOM_SHIP)) {
...
ACMD(do_drop) {
    char arg[MAX_INPUT_LENGTH];
    struct obj_data *obj;
    room_rnum RDR = NOWHERE;
    int8_t mode = SCMD_DROP;
    int dotmode, amount = 0, multi = 1, num_don_rooms;
    const char *sname;
    ...
    if (ch->getLocationRoomFlag(ROOM_GARDEN1) || ch->getLocationRoomFlag(ROOM_GARDEN2)) {
        send_to_char(ch, "You cannot do that in a garden.\r\n");
        return;
    }
    ...
    if (is_number(arg)) {
        ...
        if (!strcasecmp("zenni", arg) || !strcasecmp("gold", arg)) {
            ...
            send_to_char(ch,
                         "The save room you are in cannot hold any more items! (150 max, count of items in containers is halved)\r\n");
        } else {
            for (int i = 0; i < multi && obj; i++) {
                amount += perform_drop(ch, obj, mode, sname, RDR);
                obj = get_obj_in_list_vis(ch, arg, nullptr, ch->getContents());
            }
        }
    } else {
        ...
        if (dotmode == FIND_ALL) {
            if (cont.empty()) {
                ...
                if (fail) {
                    send_to_char(ch,
                                 "Some of the items couldn't be dropped into this save room. It is too full. (150 max, containers halve the count inside)\r\n");
                }
            }
            ...
            if (!(obj = get_obj_in_list_vis(ch, arg, nullptr, cont))) {
                send_to_char(ch, "You don't seem to have any %ss.\r\n", arg);
            } else {
                bool fail = false;
                while (obj) {
                    amount += perform_drop(ch, obj, mode, sname, RDR);
                    obj = get_obj_in_list_vis(ch, arg, nullptr, cont);
                }
                if (fail) {
                    send_to_char(ch,
                                 "Some of the items couldn't be dropped into this save room. It is too full. (150 max, containers halve the count inside)\r\n");
            ...
            if (!(obj = get_obj_in_list_vis(ch, arg, nullptr, cont))) {
                ...
                send_to_char(ch,
                             "The item couldn't be dropped into this save room. It is too full. (150 max, containers halve the count inside)\r\n");
            } else {
                amount += perform_drop(ch, obj, mode, sname, RDR);
            }
        }
...
void handle_give_multiple(struct char_data *ch, int amount, char *arg, char *argument) {
    struct char_data *vict;
    struct obj_data *obj;
...
void handle_give_objects(struct char_data *ch, char *arg, char *argument) {
    ...
    if (dotmode == FIND_INDIV) {
        ...
        handle_give_all(ch, vict, arg, dotmode);
    }
}
...
ACMD(do_give) {
    ...
    if (is_number(arg)) {
        ...
        if (strcasecmp("zenni", arg) == 0 || strcasecmp("gold", arg) == 0) {
            handle_give_gold(ch, amount, argument);
            return;
        } else {
            handle_give_multiple(ch, amount, arg, argument);
            return;
        }
    }
...
ACMD(do_drink) {
    ...
    if (!(temp = get_obj_in_list_vis(ch, arg, nullptr, ch->getContents()))) {
...
ACMD(do_eat) {
    ...
    if (!(food = get_obj_in_list_vis(ch, arg, nullptr, ch->getContents()))) {
        send_to_char(ch, "You don't seem to have %s %s.\r\n", AN(arg), arg);
        return;
    }
    ...
    if (GET_OBJ_VNUM(food) >= MEAL_START && GET_OBJ_VNUM(food) <= MEAL_LAST && 
        (!ch->getLocationRoomFlag(ROOM_AL) && !ch->getLocationRoomFlag(ROOM_RHELL))) {
...
ACMD(do_pour) {
    ...
    if (subcmd == SCMD_POUR) {
        ...
        if (!(from_obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->getContents()))) {
            send_to_char(ch, "You can't find it!\r\n");
            return;
        }
    ...
    if (subcmd == SCMD_FILL) {
        ...
        if (!(to_obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->getContents()))) {
            send_to_char(ch, "You can't find it!\r\n");
            return;
        }
    ...
    if (subcmd == SCMD_POUR) {    /* pour */
        ...
        if (!(to_obj = get_obj_in_list_vis(ch, arg2, nullptr, ch->getContents()))) {
            send_to_char(ch, "You can't find it!\r\n");
            return;
        }
        if ((GET_OBJ_TYPE(to_obj) != ITEM_DRINKCON) &&
...
using WearMapping = std::tuple<int, int, int>;

// Define a vector of wear mappings
static const std::vector<WearMapping> wearMappings = {
    {ITEM_WEAR_FINGER, WEAR_FINGER_R, WEAR_FINGER_R},
    {ITEM_WEAR_NECK, WEAR_NECK_1, WEAR_NECK_1},
    {ITEM_WEAR_BODY, WEAR_BODY, WEAR_BODY},
    {ITEM_WEAR_HEAD, WEAR_HEAD, WEAR_HEAD},
    {ITEM_WEAR_LEGS, WEAR_LEGS, WEAR_LEGS},
    {ITEM_WEAR_FEET, WEAR_FEET, WEAR_FEET},
    {ITEM_WEAR_HANDS, WEAR_HANDS, WEAR_HANDS},
    {ITEM_WEAR_ARMS, WEAR_ARMS, WEAR_ARMS},
    {ITEM_WEAR_SHIELD, WEAR_WIELD2, WEAR_WIELD2},
    {ITEM_WEAR_ABOUT, WEAR_ABOUT, WEAR_ABOUT},
    {ITEM_WEAR_WAIST, WEAR_WAIST, WEAR_WAIST},
    {ITEM_WEAR_WRIST, WEAR_WRIST_R, WEAR_WRIST_R},
    {ITEM_WEAR_HOLD, WEAR_WIELD2, WEAR_WIELD2},
    {ITEM_WEAR_PACK, WEAR_BACKPACK, WEAR_BACKPACK},
    {ITEM_WEAR_EAR, WEAR_EAR_R, WEAR_EAR_R},
    {ITEM_WEAR_SH, WEAR_SH, WEAR_SH},
    {ITEM_WEAR_EYE, WEAR_EYE, WEAR_EYE},
};
...
int find_eq_pos(struct char_data *ch, struct obj_data *obj, char *arg) {
    ...
    if (!arg || !*arg) {
        for (const auto& [itemWearFlag, bodyWearFlag, wearLocation] : wearMappings) {
        if (CAN_WEAR(obj, itemWearFlag) && BODY_FLAGGED(ch, bodyWearFlag)) {
            return wearLocation;
        }
    }
...
bool try_wear_item(struct char_data *ch, struct obj_data *obj, int where) {
    if (GET_WIS(ch) < GET_OBJ_LEVEL(obj)) {
        act("$p: you are not experienced enough to use that.", false, ch, obj, nullptr, TO_CHAR);
...
static void wear_items_in_inventory(struct char_data *ch, char *arg1, int dotmode) {
    int items_worn = 0;

    for (auto obj : IterRef(ch->getContents())) {
        int where = find_eq_pos(ch, obj, nullptr);
        if (CAN_SEE_OBJ(ch, obj) && where >= 0) {
            if (try_wear_item(ch, obj, where)) {
                items_worn++;
            }
        }
    }

    if (dotmode == FIND_ALL && items_worn == 0) {
        send_to_char(ch, "You don't seem to have anything wearable.\r\n");
    }
}

static void wear_specific_items(struct char_data *ch, char *arg1) {
    int number = get_number(&arg1); // Get the number prefix from arg1, e.g., '2.sword'
    int count = 0;

    for (auto obj : IterRef(ch->getContents())) {
        // Check if the object's name matches the argument and is visible to the character
        if (isname(arg1, obj->name) && (CAN_SEE_OBJ(ch, obj) || GET_OBJ_TYPE(obj) == ITEM_LIGHT)) {
            count++;

            // If we reach the specific numbered item (like 2.sword)
            if (count == number) {
                int where = find_eq_pos(ch, obj, nullptr);
                if (where >= 0) {
                    try_wear_item(ch, obj, where);
                } else {
                    act("You can't wear $p.", false, ch, obj, nullptr, TO_CHAR);
                }
                
                // Reset for another potential match, in case there are more items with the same name
                number++;
            }
        }
    }
...
void wear_single_item(struct char_data *ch, char *arg1, char *arg2) {
    struct obj_data *obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->getContents());

    if (!obj) {
        send_to_char(ch, "You don't seem to have %s %s.\r\n", AN(arg1), arg1);
    } else if (try_wear_item(ch, obj, find_eq_pos(ch, obj, arg2)) == false) {
        if (!*arg2) {
            act("You can't wear $p.", false, ch, obj, nullptr, TO_CHAR);
        }
    }
...
ACMD(do_wear) {
    ...
    switch (dotmode) {
        case FIND_ALL:
            wear_items_in_inventory(ch, arg1, dotmode);
            break;
        ...
        case FIND_ALLDOT:
            if (!*arg1) {
                ...
                wear_specific_items(ch, arg1);
            }
            break;

        case FIND_INDIV:
        default:
            wear_single_item(ch, arg1, arg2);
            break;
    }
}
...
ACMD(do_sac) {
    ...
    if (!(j = get_obj_in_list_vis(ch, arg, nullptr, ch->getLocationObjects())) &&
        (!(j = get_obj_in_list_vis(ch, arg, nullptr, ch->getContents())))) {
