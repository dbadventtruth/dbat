...
int parse_stats(struct descriptor_data *d, char *arg) {
  ...
  case STAT_PARSE_MENU:
    ...
    break;
  case STAT_GET_STR:
    char_stat_set(ch, "strength",
                  stats_assign_stat(char_stat_get(ch, "strength"), arg, d));
    stats_disp_menu(d);
    break;
  case STAT_GET_INT:
    char_stat_set(ch, "intelligence",
                  stats_assign_stat(char_stat_get(ch, "intelligence"), arg, d));
    stats_disp_menu(d);
    break;
  case STAT_GET_WIS:
    char_stat_set(ch, "wisdom",
                  stats_assign_stat(char_stat_get(ch, "wisdom"), arg, d));
    stats_disp_menu(d);
    break;
  case STAT_GET_DEX:
    char_stat_set(ch, "agility",
                  stats_assign_stat(char_stat_get(ch, "agility"), arg, d));
    stats_disp_menu(d);
    break;
  case STAT_GET_CON:
    char_stat_set(ch, "constitution",
                  stats_assign_stat(char_stat_get(ch, "constitution"), arg, d));
    stats_disp_menu(d);
    break;
  case STAT_GET_CHA:
    char_stat_set(ch, "speed",
                  stats_assign_stat(char_stat_get(ch, "speed"), arg, d));
    stats_disp_menu(d);
    break;
  default:
...
int stats_disp_menu(struct descriptor_data *d) {
  send_to_char(d->character,
               "\r\n"
               "@W-<[@y==========@B[ @YCWG @B]@y==========@W]>-\r\n"
               " <| Total Points Left: @m%3d@W    |>     You should select the "
               "letter of the score you\r\n"
               " <|                           |>     wish to adjust.  When "
               "prompted, enter the new score,\r\n"
               " <| = Select a stat:          |>     NOT the amount to add.  "
               "NOTE: If you quit before you\r\n"
               " <| @BS@W) @rStrength     : @m%3d@W     |>     assign all the "
               "points, you will lose them forever.\r\n"
               " <| @BD@W) @rDexterity    : @m%3d@W     |>     If your points "
               "are at zero, you may still reassign\r\n"
               " <| @BN@W) @rConstitution : @m%3d@W     |>     points by "
               "lowering any statistic, then add those\r\n"
               " <| @BI@W) @rIntelligence : @m%3d@W     |>     points to the "
               ...
               "-<[@y===========================@W]>-@n\r\n"
               "\r\n",
               OLC_VAL(d), char_stat_get(d->character, "strength"),
