...
int save_board(struct board_info *ts) {
  ...
  fprintf(fl, "Board File\n%d %d %d %d %d\n", READ_LVL(ts), WRITE_LVL(ts),
          REMOVE_LVL(ts), BOARD_MNUM(ts), CURRENT_BOARD_VER);
    ...
    if (*buf == 'S' && BOARD_VERSION(temp_board) != CURRENT_BOARD_VER) {
      ...
      if (sscanf(buf, "S %d %s %d ", &mnum, poster_name, &timestamp) == 3) {
        ...
        if ((get_name_by_id(poster) == NULL) &&
            (BOARD_VERSION(temp_board) != CURRENT_BOARD_VER)) {
          free(memboard);
        } else if ((poster_name == NULL) &&
                   (BOARD_VERSION(temp_board) == CURRENT_BOARD_VER)) {
          free(memboard);
        } else {
            ...
            for (bmsg = BOARD_MESSAGES(temp_board), sflag = 0; bmsg && !sflag;
              ...
              if (MESG_TIMESTAMP(bmsg) == MEMORY_TIMESTAMP(memboard) &&
                  (mnum == ((MESG_TIMESTAMP(bmsg) % 301 +
                             get_id_by_name(MESG_POSTER_NAME(bmsg)) % 301) %
                            301))) {
                sflag = 1;
              }
            }
  ...
  *name = '\0';
  ...
  if ((board_vnum = real_object(num)) == NOTHING) {
    ...
    send_to_char(ch, "@rO@b===================================================="
                     "========================@rO@n\n");
    send_to_char(ch,
                 "     @D[@GX@D] means you have read the message, @D[@RX@D] "
                 "means you have not.\r\n"
                 "     @WUsage@D:@CREAD@D/@cREMOVE @D<@Wmessg #@D>@W, "
                 "@CRESPOND @D<@Wmessg #@D>@W, @CWRITE @D<@Wheader@D>@W.@n\r\n"
                 "     @CVieworder@W, this changes the order in which posts "
                 "are listed to you.@n\r\n");
  } else {
    ...
    send_to_char(
        ...
        "@rO@b================================================================="
        "===========@rO@n\n"
        "     @D[@GX@D] means you have read the message, @D[@RX@D] means you "
        "have not.\r\n"
        "     @WUsage@D:@CREAD@D/@cREMOVE @D<@Wmessg #@D>@W, @CRESPOND "
        "@D<@Wmessg #@D>@W, @CWRITE @D<@Wheader@D>@W.@n\r\n"
        "     @CVieworder@W, this changes the order in which posts are listed "
        "to you.@n\r\n"
        "     "
        "@D----------------------------------------------------------------\n");
    extract_obj(obj);
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    if (msgcount < 1) {
      ...
      sprintf(buf + strlen(buf),
              "@D[%s] (@C%2d@D) : @W%6.10s @D(@G%-10s@D) ::@w %-45s\r\n",
              yesno ? "@GX@D" : "@RX@D", ++msgcount, tmstr, CAP(name),
              MESG_SUBJECT(message) ? MESG_SUBJECT(message) : "No Subject");
    }
    if (PRF_FLAGGED(ch, PRF_VIEWORDER)) {
...
void board_display_msg(obj_vnum board_vnum, struct char_data *ch, int arg) {
  ...
  if (BOARD_VERSION(thisboard) != CURRENT_BOARD_VER)
    ...
    mem = ((MESG_TIMESTAMP(message) % 301 +
            get_id_by_name(MESG_POSTER_NAME(message)) % 301) %
           301);
  /*make the new node */
...
int mesglookup(struct board_msg *message, struct char_data *ch,
  ...
  /* now, we check the mem slot. If its null, we return no, er.. 0..
     if its full, we double check against the timestamp and reader -mislabled as
     poster, but who cares... if they're not true, we go to the linked next
     slot, and repeat */
...
void write_board_message(obj_vnum board_vnum, struct char_data *ch, char *arg) {
  ...
  if (strlen(arg) > 46) {
    send_to_char(ch, "Your subject can only be 45 characters long(including "
                     "colorcode).\r\n");
    return;
  }
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWIMM > GET_BOARD(d->character, 1) && board_vnum == 3098)
      send_to_char(d->character,
                   "\r\n@GThere is a new Immortal Board Post.@n\r\n");
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWBUI > GET_BOARD(d->character, 4) && board_vnum == 3090)
      send_to_char(d->character,
                   "\r\n@GThere is a new Builder Board Post.@n\r\n");
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWDUO > GET_BOARD(d->character, 3) && board_vnum == 3099)
      send_to_char(d->character,
...
void remove_board_msg(obj_vnum board_vnum, struct char_data *ch, int arg) {
  ...
  if ((board_vnum = real_object(num)) == NOTHING) {
    ...
    if (OBJ_FLAGGED(obj, ITEM_CBOARD)) {
      ...
      } else if (GET_ADMLEVEL(ch) < REMOVE_LVL(thisboard) &&
                 strcmp(GET_NAME(ch), MESG_POSTER_NAME(cur))) {
        send_to_char(ch, "You can't remove other people's messages.\r\n");
        extract_obj(obj);
        return;
      }
    } else if (!OBJ_FLAGGED(obj, ITEM_CBOARD)) {
      if (GET_ADMLEVEL(ch) < REMOVE_LVL(thisboard) &&
          strcmp(GET_NAME(ch), MESG_POSTER_NAME(cur))) {
        send_to_char(ch, "You can't remove other people's messages.\r\n");
        extract_obj(obj);
        return;
